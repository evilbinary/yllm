...
⚠️ **早期"1.6x 差距"的解读有误**(2026-09 复测修正): 上表 serve r1 用的是
**16 线程**, 而 16 线程本身就超订降速(见 AVX2 表: 8T 26.9 vs 16T 21.3)。
把线程数差异算进"框架开销"是混淆变量。

**同等线程下的真实差距很小**:

- TinyLlama-1.1B, 同 8 线程: gen 26.8 vs serve r1 23.2(+6ms/步, ~14%,
  见下"HTTP / 会话框架"节; 其中采样参数 bug 修复后进一步收窄)。
- **MiniCPM5-2B(2.6B, avx2, 12 线程, ranks=1, temp=0, 178 decode tok,
  2026-09-09 实测)**: gen 直跑 **9.19 tok/s**, serve 全链路
  (HTTP→router→server→rank) **8.87~9.19 tok/s —— 无差异**。
  框架开销(~ms 级)相对 110ms/步 的推理耗时完全摊薄; 二次请求 prefill 仅
  1 token(会话 KV 前缀复用生效)。

serve 真正的额外代价只在 **PP 多段(ranks≥2)**: 分段串行 + 每步传输
(见"同等线程对比"与"分布式 PP"节)。
...
- 直跑(gen) vs 服务(serve):**同线程数下 r1 差距很小**(1.1B +14%, 2.6B 实测无差异);
  早期"1.6x"是 16 线程超订的混淆。serve 的额外代价只在 PP 多段
  (分段串行 + 传输);分布式目标是跨机/大模型内存扩展, 单机小模型上分段无性能收益。
