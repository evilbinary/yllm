...
static int ctl_target_port(ServeConfig* cfg, const char* target)
{
    ...
    if (strcmp(target, "s0") == 0) return cfg->server_port;
    if (strcmp(target, "r0") == 0) return cfg->rank_port_base;
...
int cmd_ctl(ServeConfig* cfg, int argc, char** argv)
{
    ...
    if (pos[2]) need_groups = atoi(pos[2]);
    if (!cmd) {
        fprintf(stderr, "usage: yllm ctl [--target <rank-0|r0|server|s0|router|rt|supervisor|sv>] "
                        "--cmd <PING|STAT|DRAIN|QUIT|SCALE|QUERY_SERVERS|status|stop> [--need-groups N]\n");
