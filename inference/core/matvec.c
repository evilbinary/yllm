...
static inline void q4k_unpack_sc(const uint8_t* scp, uint8_t sc[8], uint8_t mn[8])
{
    sc[0] = (uint8_t)(scp[0] & 63); mn[0] = (uint8_t)(scp[4] & 63);
    sc[1] = (uint8_t)(scp[1] & 63); mn[1] = (uint8_t)(scp[5] & 63);
    sc[2] = (uint8_t)(scp[2] & 63); mn[2] = (uint8_t)(scp[6] & 63);
    sc[3] = (uint8_t)(scp[3] & 63); mn[3] = (uint8_t)(scp[7] & 63);
    sc[4] = (uint8_t)((scp[8] & 15) | ((scp[0] >> 6) << 4));
    mn[4] = (uint8_t)((scp[8] >> 4) | ((scp[4] >> 6) << 4));
    sc[5] = (uint8_t)((scp[9] & 15) | ((scp[1] >> 6) << 4));
    mn[5] = (uint8_t)((scp[9] >> 4) | ((scp[5] >> 6) << 4));
    sc[6] = (uint8_t)((scp[10] & 15) | ((scp[2] >> 6) << 4));
    mn[6] = (uint8_t)((scp[10] >> 4) | ((scp[6] >> 6) << 4));
    sc[7] = (uint8_t)((scp[11] & 15) | ((scp[3] >> 6) << 4));
    mn[7] = (uint8_t)((scp[11] >> 4) | ((scp[7] >> 6) << 4));
}
...
void q4k_block(float* y, const uint8_t* blk, uint32_t stride)
{
    ...
    float min = _mm_cvtss_f32(_mm_shuffle_ps(fd, fd, _MM_SHUFFLE(1, 1, 1, 1)));
#else
    ...
    for (g = 0; g < 8; g++) {
        __m256 vd = _mm256_set1_ps(d * (float)sc8[g]);
        __m256 vm = _mm256_set1_ps(min * (float)mn8[g]);
        __m256i raw = _mm256_loadu_si256((const __m256i*)(qs + (g >> 1) * 32));
        __m256i nib = (g & 1u)
            ? _mm256_and_si256(_mm256_srli_epi16(raw, 4), nibble)
            : _mm256_and_si256(raw, nibble);
        float* yp = y + (size_t)g * 32;
        __m128i l = _mm256_castsi256_si128(nib);
        __m128i h = _mm256_extracti128_si256(nib, 1);
        _mm256_storeu_ps(yp,      _mm256_fmsub_ps(_mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(l)), vd, vm));
        _mm256_storeu_ps(yp +  8, _mm256_fmsub_ps(_mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(_mm_srli_si128(l, 8))), vd, vm));
        _mm256_storeu_ps(yp + 16, _mm256_fmsub_ps(_mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(h)), vd, vm));
        _mm256_storeu_ps(yp + 24, _mm256_fmsub_ps(_mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(_mm_srli_si128(h, 8))), vd, vm));
    }
#else
    {
        float d = f16_to_f32(((const uint16_t*)blk)[0]);
        float min = f16_to_f32(((const uint16_t*)blk)[1]);
        for (g = 0; g < 8; g++) {
            float d1 = d * (float)sc8[g];
            float m1 = min * (float)mn8[g];
            const uint8_t* q = qs + (g >> 1) * 32;
            uint32_t e;
            for (e = 0; e < 32; e++) {
                uint8_t nib = (g & 1u) ? (uint8_t)(q[e] >> 4) : (uint8_t)(q[e] & 0xF);
...
static inline __m256 q6_acc_chunk(__m256 acc, const uint8_t* ql_c, const uint8_t* qh_c,
                                  const float* xp_c, const int8_t* sc, int is, float d)
{
    ...
    acc = _mm256_fmadd_ps(q6_group_dot(e_q1, xp_c + 0),   _mm256_set1_ps(d * (float)sc[is + 0]), acc);
    acc = _mm256_fmadd_ps(q6_group_dot(o_q1, xp_c + 16),  _mm256_set1_ps(d * (float)sc[is + 1]), acc);
    acc = _mm256_fmadd_ps(q6_group_dot(e_q2, xp_c + 32),  _mm256_set1_ps(d * (float)sc[is + 2]), acc);
    acc = _mm256_fmadd_ps(q6_group_dot(o_q2, xp_c + 48),  _mm256_set1_ps(d * (float)sc[is + 3]), acc);
    acc = _mm256_fmadd_ps(q6_group_dot(e_q3, xp_c + 64),  _mm256_set1_ps(d * (float)sc[is + 4]), acc);
    acc = _mm256_fmadd_ps(q6_group_dot(o_q3, xp_c + 80),  _mm256_set1_ps(d * (float)sc[is + 5]), acc);
    acc = _mm256_fmadd_ps(q6_group_dot(e_q4, xp_c + 96),  _mm256_set1_ps(d * (float)sc[is + 6]), acc);
    return _mm256_fmadd_ps(q6_group_dot(o_q4, xp_c + 112), _mm256_set1_ps(d * (float)sc[is + 7]), acc);
}
#endif
...
static void q8k_quant_i8(const float* x, int8_t* xq, float* xs, uint32_t n)
{
    ...
    for (b = 0; b < nb; b++) {
...
#elif defined(__AVX2__)
        {
            ...
            __m128i p16 = _mm_packs_epi32(_mm256_castsi256_si128(qi), _mm256_extracti128_si256(qi, 1));
            _mm_storel_epi64((__m128i*)(xqb + i), _mm_packs_epi16(p16, p16));
            }
        }
#else
        for (i = 0; i < 256; i++) {
...
int dequant_mat_f16(uint16_t* dst, const uint8_t* w, uint32_t out, uint32_t in, uint32_t dtype)
{
    float* tmp;
    if (dtype == DT_W4B64) {
        tmp = (float*)malloc((size_t)out * in * sizeof(float));
        if (!tmp) return -1;
        if (dequant_mat_f32(tmp, w, out, in, DT_W4B64) != 0) { free(tmp); return -1; }
        f32_to_f16_buf(tmp, dst, (size_t)out * in);
        free(tmp);
        return 0;
    }
    {
...
int dequant_mat_f32(float* dst, const uint8_t* w, uint32_t out, uint32_t in, uint32_t dtype)
{
    ...
    uint32_t nb = in / 256;
    if (dtype == DT_W4B64) {
        uint32_t hu, bn, h, bl, j, oc_i, ic_i;
        if (!w4b64_bytes(out, in)) return -1;
        hu = (out + 7u) / 8u;
        bn = in / W4B64_BLK;
        memset(dst, 0, (size_t)out * in * 4);
        for (h = 0; h < hu; h++) {
            for (bl = 0; bl < bn; bl++) {
                ...
                for (j = 0; j < W4B64_LU; j++) {
                    ...
                    for (oc_i = 0; oc_i < 4; oc_i++) {
                        uint32_t oc0 = h * 8u + oc_i;
                        uint32_t oc1 = oc0 + 4;
                        for (ic_i = 0; ic_i < 4; ic_i++) {
                            uint32_t ic = bl * W4B64_BLK + j * 4u + ic_i;
                            uint8_t by = cell[oc_i * 4 + ic_i];
                            if (oc0 < out)
                                dst[(size_t)oc0 * in + ic] =
                                    wscale[oc_i] * (float)((int)(by >> 4) - 8);
                            if (oc1 < out)
                                dst[(size_t)oc1 * in + ic] =
                                    wscale[oc_i + 4] * (float)((int)(by & 15) - 8);
                        }
                    }
                }
            }
...
#ifdef __AVX2__
static inline float hsum_i32_avx2(__m256i v)
{
    return hsum_avx2(_mm256_cvtepi32_ps(v));
}

static inline __m256 q4k_acc_block(__m256 acc, float* cmin,
    const uint8_t* blk, const int8_t* xp0, const float* sxb, float xs_b)
{
    ...
    float dmin = f16_to_f32(((const uint16_t*)blk)[1]);
#endif
    float dsc = d * xs_b, msc = dmin * xs_b;
    uint8_t sc8[8], mn8[8];
    float d1[8], m1[8];
    int ii, j, is;
    ...
    const int8_t* xp = xp0;
    q4k_unpack_sc(blk + 4, sc8, mn8);
    for (ii = 0; ii < 8; ii++) { d1[ii] = dsc * (float)sc8[ii]; m1[ii] = msc * (float)mn8[ii]; }
    is = 0;
    for (j = 0; j < 4; j++) {
        ...
        __m256i p1 = _mm256_maddubs_epi16(qv1, xv1);
        __m256i p2 = _mm256_maddubs_epi16(qv2, xv2);
        acc = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_madd_epi16(p1, ones_i16)), _mm256_set1_ps(d1[is]), acc);
        acc = _mm256_fmadd_ps(_mm256_cvtepi32_ps(_mm256_madd_epi16(p2, ones_i16)), _mm256_set1_ps(d1[is + 1]), acc);
        *cmin -= sxb[is] * m1[is] + sxb[is + 1] * m1[is + 1];
        xp += 64;
        q += 32;
        is += 2;
    }
    ...
    q8k_quant_i8(x, xq8, xs, in);
    {
        ...
        for (b = 0; b < nb; b++) {
            ...
            for (g = 0; g < 8; g++) {
                ...
                sxg[(size_t)b * 8 + g] = hsum_i32_avx2(
                    _mm256_madd_epi16(_mm256_maddubs_epi16(ones_u8, xv), ones_i16));
            }
        }
    }
    {
        ...
        for (oo = 0; oo < n2; oo += 2) {
            ...
            for (b = 0; b < nb; b++) {
                ...
                if (b + 1 < nb) {
                    _mm_prefetch((const char*)(blk0 + 144), _MM_HINT_T0);
                    _mm_prefetch((const char*)(blk1 + 144), _MM_HINT_T0);
                }
                acc0 = q4k_acc_block(acc0, &c0, blk0, xp, sxb, xs[b]);
                acc1 = q4k_acc_block(acc1, &c1, blk1, xp, sxb, xs[b]);
            }
            y[oo] = hsum_avx2(acc0) + c0;
            y[oo + 1] = hsum_avx2(acc1) + c1;
        }
        if (out & 1u) {
            oo = out - 1u;
            const uint8_t* row = w + (size_t)oo * rowb;
            ...
            uint32_t b;
            for (b = 0; b < nb; b++) {
                acc = q4k_acc_block(acc, &c, row + (size_t)b * 144,
                                    xq8 + (size_t)b * 256, sxg + (size_t)b * 8, xs[b]);
            }
            y[oo] = hsum_avx2(acc) + c;
        }
    }
        ...
        for (oo = 0; oo < n2; oo += 2) {
            ...
            for (b = 0; b < nb; b++) {
                ...
                for (chunk = 0; chunk < 2; chunk++) {
                    ...
                    int is = chunk * 8;
                    acc0 = q6_acc_chunk(acc0, blk0 + chunk * 64, blk0 + 128 + chunk * 32,
                                        xp_c, (const int8_t*)(blk0 + 192), is, d0);
                    acc1 = q6_acc_chunk(acc1, blk1 + chunk * 64, blk1 + 128 + chunk * 32,
                                        xp_c, (const int8_t*)(blk1 + 192), is, d1);
                }
            }
            y[oo] = hsum_avx2(acc0);
            y[oo + 1] = hsum_avx2(acc1);
        }
        if (out & 1u) {
            oo = out - 1u;
            const uint8_t* row = w + (size_t)oo * rowb;
            ...
            for (b = 0; b < nb; b++) {
                ...
                int chunk;
                for (chunk = 0; chunk < 2; chunk++)
                    acc_v = q6_acc_chunk(acc_v, blk + chunk * 64, blk + 128 + chunk * 32,
                                         xb + chunk * 128, (const int8_t*)(blk + 192), chunk * 8, d);
            }
            y[oo] = hsum_avx2(acc_v);
        }
    }
...
{
    uint32_t hu, bn;
    if (out == 0 || in == 0 || (in % W4B64_BLK) != 0) return 0;
    hu = (out + W4B64_HP - 1u) / W4B64_HP;
    bn = in / W4B64_BLK;
    return (size_t)hu * bn * W4B64_PANEL_BYTES;
}

size_t w4b64_row_bytes(uint32_t in)
{
    /* 旧行主序 API 已废; 保留符号以免外部链接失败 */
    (void)in;
    return 0;
}
...
int w4b64_prefer_i8mm(void)
{
    ...
    return g_w4_prefer_i8mm;
#else
    (void)g_w4_prefer_i8mm;
    return 0;
#endif
}
...
{
    g_w4_prefer_i8mm = on ? 1 : 0;
    return w4b64_prefer_i8mm();
}
...
static void w4b64_quant_row_block(int8_t* qout /*[64]*/, float* scale, const float* x)
{
    uint32_t i;
    float amax = 0.0f;
    ...
    *scale = (amax > 1e-8f) ? (amax / 8.0f) : 1.0f;
    {
        float inv = 1.0f / *scale;
        for (i = 0; i < W4B64_BLK; i++) {
            int q = (int)lrintf(x[i] * inv);
            if (q < -8) q = -8;
            if (q > 7) q = 7;
            qout[i] = (int8_t)q;
        }
...
/* Arm82 cell: byte b → hi=OC(b/4), lo=OC(b/4+4), IC=b%4; nibbles = signed+8 */
static void w4b64_pack_arm82_cell(uint8_t cell[16], const int8_t* w8 /*[8][4] OC-major*/)
{
    int oc_i, ic_i;
    for (oc_i = 0; oc_i < 4; oc_i++) {
        for (ic_i = 0; ic_i < 4; ic_i++) {
            int q0 = (int)w8[oc_i * 4 + ic_i] + 8;
            int q1 = (int)w8[(oc_i + 4) * 4 + ic_i] + 8;
            if (q0 < 0) q0 = 0;
            if (q0 > 15) q0 = 15;
            if (q1 < 0) q1 = 0;
            if (q1 > 15) q1 = 15;
            cell[oc_i * 4 + ic_i] = (uint8_t)((q0 << 4) | q1);
        }
    }
}
...
int w4b64_pack_mat_f32(uint8_t* dst, const float* src, uint32_t out, uint32_t in)
{
    ...
    size_t need;
    if (!dst || !src) return -1;
    need = w4b64_bytes(out, in);
    if (!need) return -1;
    hu = (out + W4B64_HP - 1u) / W4B64_HP;
    bn = in / W4B64_BLK;
    for (h = 0; h < hu; h++) {
        for (bl = 0; bl < bn; bl++) {
            uint8_t* panel = dst + ((size_t)h * bn + bl) * W4B64_PANEL_BYTES;
            float scales[8];
            float zeros[8];
            int8_t qblk[8][W4B64_BLK];
            memset(scales, 0, sizeof(scales));
            memset(zeros, 0, sizeof(zeros));
            memset(qblk, 0, sizeof(qblk));
            for (oc_i = 0; oc_i < 8; oc_i++) {
                uint32_t oc = h * 8u + oc_i;
                if (oc >= out) continue;
                w4b64_quant_row_block(qblk[oc_i], &scales[oc_i],
                                     src + (size_t)oc * in + (size_t)bl * W4B64_BLK);
            }
            for (j = 0; j < W4B64_LU; j++) {
                int8_t w8[32];
                uint8_t cell[16];
                for (oc_i = 0; oc_i < 8; oc_i++)
                    for (ic_i = 0; ic_i < 4; ic_i++)
                        w8[oc_i * 4 + ic_i] = qblk[oc_i][j * 4 + ic_i];
                w4b64_pack_arm82_cell(cell, w8);
                memcpy(panel + (size_t)j * 16, cell, 16);
            }
            memcpy(panel + W4B64_LU * 16, scales, 32);
            memcpy(panel + W4B64_LU * 16 + 32, zeros, 32);
        }
    }
    return 0;
}
...
int w4b64_pack_mat_q4k(uint8_t* dst, const uint8_t* src, uint32_t out, uint32_t in)
{
    ...
    float* row;
    int rc;
    if (!dst || !src || (in % 256) != 0 || !w4b64_bytes(out, in)) return -1;
    row = (float*)ymalloc((size_t)out * in * 4);
    if (!row) return -1;
    for (oo = 0; oo < out; oo++) {
        const uint8_t* r = src + (size_t)oo * nb4 * 144;
        for (b = 0; b < nb4; b++)
            q4k_block(row + (size_t)oo * in + (size_t)b * 256, r + (size_t)b * 144, 0);
    }
    rc = w4b64_pack_mat_f32(dst, row, out, in);
    free(row);
    return rc;
}
...
static void w4_act_quant_block(const float* x, int8_t* xq, float* xs, int32_t* xsum, uint32_t nb)
{
    ...
    for (b = 0; b < nb; b++) {
...
#if defined(__aarch64__)
        {
            ...
            float32x4_t vinv;
            for (i = 0; i < W4B64_BLK; i += 16) {
                amaxv = vmaxq_f32(amaxv, vabsq_f32(vld1q_f32(xb + i)));
                amaxv = vmaxq_f32(amaxv, vabsq_f32(vld1q_f32(xb + i + 4)));
                amaxv = vmaxq_f32(amaxv, vabsq_f32(vld1q_f32(xb + i + 8)));
                amaxv = vmaxq_f32(amaxv, vabsq_f32(vld1q_f32(xb + i + 12)));
            }
            amax = vmaxvq_f32(amaxv);
            scale = (amax > 1e-8f) ? (amax / 127.0f) : 1.0f;
            inv = 1.0f / scale;
            xs[b] = scale;
            vinv = vdupq_n_f32(inv);
            for (i = 0; i < W4B64_BLK; i += 16) {
                int32x4_t q0 = vmaxq_s32(vminq_s32(vcvtnq_s32_f32(vmulq_f32(vld1q_f32(xb + i), vinv)), qmax), qmin);
                int32x4_t q1 = vmaxq_s32(vminq_s32(vcvtnq_s32_f32(vmulq_f32(vld1q_f32(xb + i + 4), vinv)), qmax), qmin);
                int32x4_t q2 = vmaxq_s32(vminq_s32(vcvtnq_s32_f32(vmulq_f32(vld1q_f32(xb + i + 8), vinv)), qmax), qmin);
                int32x4_t q3 = vmaxq_s32(vminq_s32(vcvtnq_s32_f32(vmulq_f32(vld1q_f32(xb + i + 12), vinv)), qmax), qmin);
                int16x8_t n0 = vcombine_s16(vqmovn_s32(q0), vqmovn_s32(q1));
                int16x8_t n1 = vcombine_s16(vqmovn_s32(q2), vqmovn_s32(q3));
                vst1q_s8(xq + (size_t)b * W4B64_BLK + i,
                         vcombine_s8(vqmovn_s16(n0), vqmovn_s16(n1)));
                sumv = vaddq_s32(sumv, q0);
                sumv = vaddq_s32(sumv, q1);
                sumv = vaddq_s32(sumv, q2);
                sumv = vaddq_s32(sumv, q3);
            }
            xsum[b] = vaddvq_s32(sumv);
        }
#else
        amax = 0.0f;
        for (i = 0; i < W4B64_BLK; i++) {
...
#pragma omp parallel for schedule(static)
    for (h = 0; h < hu; h++) {
        ...
        for (bl = 0; bl < bn; bl++) {
            ...
            for (j = 0; j < W4B64_LU; j++) {
                ...
                for (oc_i = 0; oc_i < 8; oc_i++) {
                    ...
                    for (ic_i = 0; ic_i < 4; ic_i++)
                        iacc[oc_i] += (int32_t)x4[ic_i] * (int32_t)wj[ic_i];
                }
            }
            for (oc_i = 0; oc_i < 8; oc_i++) {
        ...
        for (oc_i = 0; oc_i < 8; oc_i++) {
            if (oc0 + oc_i < out) y[oc0 + oc_i] = acc[oc_i];
        }
    }
}
                                     ...
                                     uint32_t j, uint8x16_t mask15, int arm86)
{
    ...
    c = vld1q_u8(panel + (size_t)(j + 1) * 16);
    hi = vdotq_laneq_s32(hi, vreinterpretq_s8_u8(vshrq_n_u8(c, 4)), xv, 1);
    lo = vdotq_laneq_s32(lo, vreinterpretq_s8_u8(vandq_u8(c, mask15)), xv, 1);
    c = vld1q_u8(panel + (size_t)(j + 2) * 16);
    hi = vdotq_laneq_s32(hi, vreinterpretq_s8_u8(vshrq_n_u8(c, 4)), xv, 2);
    lo = vdotq_laneq_s32(lo, vreinterpretq_s8_u8(vandq_u8(c, mask15)), xv, 2);
    c = vld1q_u8(panel + (size_t)(j + 3) * 16);
    hi = vdotq_laneq_s32(hi, vreinterpretq_s8_u8(vshrq_n_u8(c, 4)), xv, 3);
    lo = vdotq_laneq_s32(lo, vreinterpretq_s8_u8(vandq_u8(c, mask15)), xv, 3);
    *ihi = hi;
    *ilo = lo;
}
...
#pragma omp parallel for schedule(static)
    for (h = 0; h < hu; h++) {
        ...
        for (bl = 0; bl < bn; bl++) {
            ...
            for (j = 0; j < W4B64_LU; j++) {
                ...
                int8x16_t wl = vreinterpretq_s8_u8(vandq_u8(c, mask15));
                {
                    int32_t thi[4], tlo[4];
                    int oc2, ic2;
                    int8_t whb[16], wlb[16], x4[4];
                    vst1q_s32(thi, iacc_hi);
                    vst1q_s32(tlo, iacc_lo);
                    vst1q_s8(whb, wh);
                    vst1q_s8(wlb, wl);
                    memcpy(x4, xp + j * 4, 4);
                    for (oc2 = 0; oc2 < 4; oc2++)
                        for (ic2 = 0; ic2 < 4; ic2++) {
                            thi[oc2] += (int32_t)whb[oc2 * 4 + ic2] * (int32_t)x4[ic2];
                            tlo[oc2] += (int32_t)wlb[oc2 * 4 + ic2] * (int32_t)x4[ic2];
                        }
        ...
        if (oc0 + 8u <= out) {
            vst1q_f32(y + oc0, acc_hi);
            vst1q_f32(y + oc0 + 4, acc_lo);
        } else {
            float tmp[8];
            uint32_t oc_i;
            vst1q_f32(tmp, acc_hi);
            vst1q_f32(tmp + 4, acc_lo);
            for (oc_i = 0; oc_i < 8; oc_i++)
...
static inline int32x4_t w4_smmla(int32x4_t r, int8x16_t a, int8x16_t b)
{
    return vmmlaq_s32(r, a, b);
}

/* Arm82 两格 (IC0-3 + IC4-7) → Arm86 两个 16B: hi=OC0-1/2-3, lo=OC4-5/6-7 */
static inline void w4_arm82_pair_zip(uint8x16_t c0, uint8x16_t c1, uint8x16_t* v0, uint8x16_t* v1)
{
    int32x4_t a = vreinterpretq_s32_u8(c0);
    int32x4_t b = vreinterpretq_s32_u8(c1);
    *v0 = vreinterpretq_u8_s32(vzip1q_s32(a, b));
...
static inline int32x4_t w4_smmla_e1(int32x4_t p01, int32x4_t p23)
{
    return vreinterpretq_s32_s64(vuzp2q_s64(vreinterpretq_s64_s32(p01), vreinterpretq_s64_s32(p23)));
}
    ...
    for (h = 0; h < hu; h++) {
        ...
        for (bl = 0; bl < bn; bl++) {
            ...
            for (j = 0; j < 8; j++) {
                ...
                a67 = w4_smmla(a67, xa, wl67);
            }
            {
                int32x4_t ihi = vsubq_s32(vcombine_s32(vget_low_s32(a01), vget_low_s32(a23)), corr);
                int32x4_t ilo = vsubq_s32(vcombine_s32(vget_low_s32(a45), vget_low_s32(a67)), corr);
                float32x4_t fi = vmulq_n_f32(vcvtq_f32_s32(ihi), xsc);
                float32x4_t fj = vmulq_n_f32(vcvtq_f32_s32(ilo), xsc);
                acc_hi = vmlaq_f32(acc_hi, fi, vld1q_f32(wscale));
                acc_lo = vmlaq_f32(acc_lo, fj, vld1q_f32(wscale + 4));
            }
        }
                 ...
                 uint32_t row_begin, uint32_t n_rows, uint32_t in, uint32_t out, uint32_t dtype)
{
    ...
    case DT_W4B64:
        ...
        if ((row_begin % W4B64_HP) != 0) {
            matmul_w4b64(y, x, w, n_rows, in); /* 退化: 错误布局风险, 仅兜底 */
            break;
        }
        {
            uint32_t hu0 = row_begin / W4B64_HP;
            uint32_t bn = in / W4B64_BLK;
            const uint8_t* wp = w + (size_t)hu0 * bn * W4B64_PANEL_BYTES;
            matmul_w4b64(y, x, wp, n_rows, in);
        }
        break;
...
#if defined(__aarch64__) && defined(__ARM_FEATURE_MATMUL_INT8)
            /* zip 一次一对 cell, 4-token 的 iacc 留在寄存器; 避免 8 pair 全展开把 Q 寄存器打满 */
            if (w4b64_has_i8mm()) {
                const uint8x16_t mask15 = vdupq_n_u8(15);
                ...
                for (bl = 0; bl < nb64; bl++) {
                    for (p = 0; p < np2; p++) {
                        uint32_t g0 = p * 2u;
                        for (j = 0; j < 8; j++) {
                            size_t off = (size_t)bl * W4B64_BLK + (size_t)j * 8;
                            int8_t* dst = xpk8[bl][p][j];
                            if (g0 < B)
                                memcpy(dst, xq + (size_t)g0 * in + off, 8);
                            if (g0 + 1u < B)
                                memcpy(dst + 8, xq + (size_t)(g0 + 1) * in + off, 8);
                        }
                    }
                }
                    ...
                    for (bl = 0; bl < nb64; bl++) {
                        ...
                        while (p + 2 <= np2) {
                            ...
                            for (j = 0; j < 8; j++) {
                                ...
                                a45 = w4_smmla(a45, xa, wl45);
                                a67 = w4_smmla(a67, xa, wl67);
                                b01 = w4_smmla(b01, xb, wh01);
                                b23 = w4_smmla(b23, xb, wh23);
                                b45 = w4_smmla(b45, xb, wl45);
                                b67 = w4_smmla(b67, xb, wl67);
                            }
                            {
                                uint32_t t;
                                int32x4_t th[2], tl[2], uh[2], ul[2];
                                th[0] = w4_smmla_e0(a01, a23); th[1] = w4_smmla_e1(a01, a23);
                                tl[0] = w4_smmla_e0(a45, a67); tl[1] = w4_smmla_e1(a45, a67);
                                uh[0] = w4_smmla_e0(b01, b23); uh[1] = w4_smmla_e1(b01, b23);
                                ul[0] = w4_smmla_e0(b45, b67); ul[1] = w4_smmla_e1(b45, b67);
                                for (t = 0; t < 2 && g0 + t < B; t++) {
                                    float xsc = xs[(size_t)(g0 + t) * nb64 + bl];
                                    int32x4_t corr = vdupq_n_s32(8 * xsum[(size_t)(g0 + t) * nb64 + bl]);
                                    acc_hi[g0 + t] = vmlaq_f32(acc_hi[g0 + t],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(th[t], corr)), xsc), ws_hi);
                                    acc_lo[g0 + t] = vmlaq_f32(acc_lo[g0 + t],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(tl[t], corr)), xsc), ws_lo);
                                }
                                for (t = 0; t < 2 && g0 + 2 + t < B; t++) {
                                    float xsc = xs[(size_t)(g0 + 2 + t) * nb64 + bl];
                                    int32x4_t corr = vdupq_n_s32(8 * xsum[(size_t)(g0 + 2 + t) * nb64 + bl]);
                                    acc_hi[g0 + 2 + t] = vmlaq_f32(acc_hi[g0 + 2 + t],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(uh[t], corr)), xsc), ws_hi);
                                    acc_lo[g0 + 2 + t] = vmlaq_f32(acc_lo[g0 + 2 + t],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(ul[t], corr)), xsc), ws_lo);
                                }
                            }
                            p += 2;
                        }
                        if (p < np2) {
                            ...
                            {
                                int32x4_t th0 = w4_smmla_e0(a01, a23);
                                int32x4_t th1 = w4_smmla_e1(a01, a23);
                                int32x4_t tl0 = w4_smmla_e0(a45, a67);
                                int32x4_t tl1 = w4_smmla_e1(a45, a67);
                                if (g0 < B) {
                                    float xsc = xs[(size_t)g0 * nb64 + bl];
                                    int32x4_t corr = vdupq_n_s32(8 * xsum[(size_t)g0 * nb64 + bl]);
                                    acc_hi[g0] = vmlaq_f32(acc_hi[g0],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(th0, corr)), xsc), ws_hi);
                                    acc_lo[g0] = vmlaq_f32(acc_lo[g0],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(tl0, corr)), xsc), ws_lo);
                                }
                                if (g0 + 1u < B) {
                                    float xsc = xs[(size_t)(g0 + 1) * nb64 + bl];
                                    int32x4_t corr = vdupq_n_s32(8 * xsum[(size_t)(g0 + 1) * nb64 + bl]);
                                    acc_hi[g0 + 1] = vmlaq_f32(acc_hi[g0 + 1],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(th1, corr)), xsc), ws_hi);
                                    acc_lo[g0 + 1] = vmlaq_f32(acc_lo[g0 + 1],
                                        vmulq_n_f32(vcvtq_f32_s32(vsubq_s32(tl1, corr)), xsc), ws_lo);
                                }
                            }
                        }
                    }
...
#if defined(__AVX2__) && defined(__F16C__)
    {
        ...
        for (; i + 16 <= n; i += 16) {
            acc = _mm256_fmadd_ps(_mm256_loadu_ps(a + i),
                _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*)(b + i))), acc);
            acc = _mm256_fmadd_ps(_mm256_loadu_ps(a + i + 8),
                _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*)(b + i + 8))), acc);
        }
        for (; i + 8 <= n; i += 8)
            acc = _mm256_fmadd_ps(_mm256_loadu_ps(a + i),
                _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*)(b + i))), acc);
...
#if defined(__AVX2__) && defined(__F16C__)
    {
        ...
        for (; i + 16 <= n; i += 16) {
            _mm256_storeu_ps(y + i, _mm256_fmadd_ps(va,
                _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*)(x + i))), _mm256_loadu_ps(y + i)));
            _mm256_storeu_ps(y + i + 8, _mm256_fmadd_ps(va,
                _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*)(x + i + 8))), _mm256_loadu_ps(y + i + 8)));
        }
        for (; i + 8 <= n; i += 8)
            _mm256_storeu_ps(y + i, _mm256_fmadd_ps(va,
                _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*)(x + i))), _mm256_loadu_ps(y + i)));
...
static inline void vec_scale_f32(float* y, float a, uint32_t n)
{
    uint32_t i = 0;
#if defined(__AVX2__)
    __m256 va = _mm256_set1_ps(a);
...
static inline void vec_axpy_f32(float* y, const float* x, float a, uint32_t n)
{
    uint32_t i = 0;
#if defined(__AVX2__)
    __m256 va = _mm256_set1_ps(a);
